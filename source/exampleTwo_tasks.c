...
void taskFunctionTwo(void *pvParameters);
...
/*
 * THIS EXAMPLE SHOULD GO THROUGH BUT DOESN'T TAKE INTO ACCOUNT THE 18 MS CONTEXT SWITCH
 * NECESSARY BETWEEN EACH TASK SWITCH.
 */
void createSchedulerDemoTasks(void) {
    ...
    taskC_CPUHard = (TaskModel){
        ...
        .type = TASK_TYPE_CPU,
        .deadlineType = DEADLINE_HARD,
        .releaseTime = 25000,   // Released 25ms after Task A starts
        .period = 100000,       // 100 ms
        .deadline = 100000,      // 100 ms (tight deadline to force preemption)
        .capacity = 5000,      // 5 ms of work
        .stackSize = DEMO_STACK_SIZE,
        .state = TASK_STATE_WAITING,
        .handle = NULL,
    };
    ...
    taskD_CPU_EDF = (TaskModel){
        ...
        .type = TASK_TYPE_CPU,
        .deadlineType = DEADLINE_HARD,
        .releaseTime = 25000,   // Released at the same time as Task C
        .period = 180000,       // 180 ms
        .deadline = 180000,      // 90 ms (later deadline than Task C)
